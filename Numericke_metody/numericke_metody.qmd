...
jacobiova_metoda <- function(A, b, x0 = NULL, tol = 1e-6, max_iter = 1000) {
  ...
  for (iter in 1:max_iter) {
    ...
    if (max(abs(x_new - x)) < tol) { # Kontrola konvergence
...
linear_interpolation <- function(x_data, y_data, x_eval) {
  ...
  for (k in 1:length(x_eval)) {
    ...
    if (x <= x_data[1]) { 
      y_eval[k] <- y_data[1] + (y_data[2] - y_data[1]) / (x_data[2] - x_data[1]) * (x - x_data[1]) # Extrapolace vlevo
    } 
    else if (x >= x_data[n]) {
...
```{r}
dopredna_diference <- function(f, x, h = 1e-5) return((f(x + h) - f(x)) / h)
```
...
```{r}
zpetna_diference <- function(f, x, h = 1e-5) return((f(x) - f(x - h)) / h)
```
...
```{r}
centralni_diference_1 <- function(f, x, h = 1e-5) return((f(x + h) - f(x - h)) / (2 * h))
...
```{r}
centralni_diference_2 <- function(f, x, h = 1e-5) return((f(x+h) - 2*f(x) + f(x-h)) / (h^2)) # derivace
```
...
# Použijeme uniroot pro nalezení kořenů, Můžeme mít více řešení, takže hledáme v různých intervalech
