...
InnerIntegral <- function(alpha) {
  f <- \(x) exp(-alpha * x) * sin(x) # Funkce: exponenciálně tlumený sinus
...
if (error < 1e-4) cat("Výsledek je správný! ✓\n") else cat("Chyba je příliš velká\n")

# Vytvoříme body pro graf
p_values <- seq(-5, 5, length.out = 50)
...
LSA <- function(x, y, n){ # x a y je vektor hodnot, n je stupeň polynomu
  ...
  for(i in 1:n){
    for(j in 1:n){
      A[i,j] <- sum(x^(i+j-2)) # A je matice součtů mocnin x
    }
...
data_table <- data.frame(x = x_points, y = y_points, integral_0_to_x = sapply(x_points, \(xi) find_b_for_integral(f, xi)))
cat("\nTabulka hodnot pro interpolaci:\n")
print(data_table)
# 3. Lineární interpolace
x_fine <- seq(0.05, 0.25, by = 0.001)
lagrange_values <- sapply(x_fine, \(t) Lagrange(t, x_points, y_points))
...
plot2 <- ggplot() +
  geom_line(aes(x = x_fine, y = sapply(x_fine, f)), color = "black", size = 1, alpha = 0.7) + # Původní funkce
  geom_line(aes(x = x_fine, y = lagrange_values), color = "red", size = 1) + # Interpolace a aproximace
  geom_line(aes(x = x_fine, y = poly_4_values), color = "blue", size = 2) + 
  geom_point(aes(x = x_points, y = y_points), color = "purple", size = 3) + # Body pro interpolaci
...
for(i in 1:(length(x_hodnoty)-1)){ #projdém iteraci délkou x hodnot
  #zjišťuji jestli existuje v intervalu kořen
  if(f(x_hodnoty[i])*f(x_hodnoty[i+1])<0){
